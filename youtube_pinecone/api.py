...
app.add_middleware(
    ...
    allow_origins=[
        "http://localhost:3002",
        "https://lumira-phi.vercel.app",
    ],  # Update this with your React app's URL
...
async def ask_question_endpoint(
    request: QuestionRequest, user_id: str = Depends(get_current_user)
):
    try:
        answer, vector_results = await answer_question(
            request.question, pc, user_id=user_id
        )
        return AnswerResponse(
            answer=answer, vectorResults=[result.metadata for result in vector_results]
        )
    except Exception as e:
...
async def ask_question_about_video_endpoint(
    video_id: str, request: QuestionRequest, user_id: str = Depends(get_current_user)
):
    try:
        answer, vector_results = await answer_question(
            request.question, pc, video_id=video_id, user_id=user_id
        )
        return AnswerResponse(
            answer=answer, vectorResults=[result.metadata for result in vector_results]
        )
